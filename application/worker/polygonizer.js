( function ( ) {
	...
	var computeGeometries = function ( data, width, height, depth ) {
		...
		var xF = ( 1 );
		var yF = ( width + 1 ) * ( 1 )
		var zF = ( width + 1 ) * ( height + 1 ) * ( 1 );

		var vertexInterpolation = function ( a, b ) {
			return [
			...
			for ( var y = 0; y < height; ++ y ) {
				for ( var x = 0; x < width; ++ x ) {
					...
					var values = [
						...
						data[ positions[ 1 ][ 0 ] * xF + positions[ 1 ][ 1 ] * yF + positions[ 1 ][ 2 ] * zF ],
						data[ positions[ 2 ][ 0 ] * xF + positions[ 2 ][ 1 ] * yF + positions[ 2 ][ 2 ] * zF ],
						data[ positions[ 3 ][ 0 ] * xF + positions[ 3 ][ 1 ] * yF + positions[ 3 ][ 2 ] * zF ],
						data[ positions[ 4 ][ 0 ] * xF + positions[ 4 ][ 1 ] * yF + positions[ 4 ][ 2 ] * zF ],
						data[ positions[ 5 ][ 0 ] * xF + positions[ 5 ][ 1 ] * yF + positions[ 5 ][ 2 ] * zF ],
	...
	var buildGeometriesBuffers = function ( geometries ) {
		...
		return Object.keys( geometries ).reduce( function ( geometriesBuffers, materialIndex ) {
			
			var triangles = geometries[ materialIndex ];
			...
			triangles.forEach( function ( triangle, triangleIndex ) {
				...
				indexArray[ triangleIndex * 3 + 0 ] = triangleIndex * 3 + 0;
				indexArray[ triangleIndex * 3 + 1 ] = triangleIndex * 3 + 1;
				indexArray[ triangleIndex * 3 + 2 ] = triangleIndex * 3 + 2;
				
				copy( triangle.vertices[ 0 ], positionArray, triangleIndex * 3 * 3 + 0 * 3 );
				copy( triangle.vertices[ 1 ], positionArray, triangleIndex * 3 * 3 + 1 * 3 );
				copy( triangle.vertices[ 2 ], positionArray, triangleIndex * 3 * 3 + 2 * 3 );
				
				copy( triangle.normal, normalArray, triangleIndex * 3 * 3 + 0 * 3 );
	...
	self.addEventListener( 'message', function ( e ) {
		...
		case 'polygonize':
		    ...
		    var geometries = computeGeometries( data, width, height, depth );
			var geometriesBuffers = buildGeometriesBuffers( geometries );

			var transferables = Array.prototype.concat.apply( [ ], Object.keys( geometriesBuffers ).map( function ( materialIndex ) {
